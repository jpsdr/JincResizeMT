	...
	for (int y = Y_Min; y < Y_Max; y++)
	{
		...
		for (int x = 0; x < dst_width; x++)
		{
			...
			if JincMT_CONSTEXPR(std::is_same<T, uint8_t>::value)
			{
				...
				const __m128 hsum1 = _mm_hadd_ps(_mm_hadd_ps(result1, result1), _mm_hadd_ps(result1, result1));
			...
			else
			{
				...
				dst1[x] = _mm_cvtss_f32(_mm_hadd_ps(_mm_hadd_ps(result1, result1), _mm_hadd_ps(result1, result1)));
				dst2[x] = _mm_cvtss_f32(_mm_hadd_ps(_mm_hadd_ps(result2, result2), _mm_hadd_ps(result2, result2)));
			}
	...
	for (int y = Y_Min; y < Y_Max; y++)
	{
		...
		for (int x = 0; x < dst_width; x++)
		{
			...
			if JincMT_CONSTEXPR(std::is_same<T, uint8_t>::value)
			{
				...
				const __m128 hsum1 = _mm_hadd_ps(_mm_hadd_ps(result1, result1), _mm_hadd_ps(result1, result1));
				const __m128 hsum2 = _mm_hadd_ps(_mm_hadd_ps(result2, result2), _mm_hadd_ps(result2, result2));
				const __m128 hsum3 = _mm_hadd_ps(_mm_hadd_ps(result3, result3), _mm_hadd_ps(result3, result3));
				const T final_res1 = _mm_cvtsi128_si32(_mm_packus_epi16(_mm_packus_epi32(_mm_cvtps_epi32(hsum1), _mm_setzero_si128()), _mm_setzero_si128()));
				const T final_res2 = _mm_cvtsi128_si32(_mm_packus_epi16(_mm_packus_epi32(_mm_cvtps_epi32(hsum2), _mm_setzero_si128()), _mm_setzero_si128()));
			...
			else
			{
				...
				dst1[x] = _mm_cvtss_f32(_mm_hadd_ps(_mm_hadd_ps(result1, result1), _mm_hadd_ps(result1, result1)));
				dst2[x] = _mm_cvtss_f32(_mm_hadd_ps(_mm_hadd_ps(result2, result2), _mm_hadd_ps(result2, result2)));
				dst3[x] = _mm_cvtss_f32(_mm_hadd_ps(_mm_hadd_ps(result3, result3), _mm_hadd_ps(result3, result3)));
			}
	...
	for (int y = Y_Min; y < Y_Max; y++)
	{
		...
		for (int x = 0; x < dst_width; x++)
		{
			...
			if JincMT_CONSTEXPR(std::is_same<T, uint8_t>::value)
			{
				for (int ly = 0; ly < filter_size; ++ly)
				{
					for (int lx = 0; lx < filter_size; lx += 4)
					{
						...
						result2 = _mm_add_ps(result2, _mm_mul_ps(src_ps2, coeff));
						result3 = _mm_add_ps(result3, _mm_mul_ps(src_ps3, coeff));
						result4 = _mm_add_ps(result4, _mm_mul_ps(src_ps4, coeff));
					}
				...
				const __m128 hsum2 = _mm_hadd_ps(_mm_hadd_ps(result2, result2), _mm_hadd_ps(result2, result2));
				const __m128 hsum3 = _mm_hadd_ps(_mm_hadd_ps(result3, result3), _mm_hadd_ps(result3, result3));
				const __m128 hsum4 = _mm_hadd_ps(_mm_hadd_ps(result4, result4), _mm_hadd_ps(result4, result4));
				const T final_res1 = _mm_cvtsi128_si32(_mm_packus_epi16(_mm_packus_epi32(_mm_cvtps_epi32(hsum1), _mm_setzero_si128()), _mm_setzero_si128()));
				const T final_res2 = _mm_cvtsi128_si32(_mm_packus_epi16(_mm_packus_epi32(_mm_cvtps_epi32(hsum2), _mm_setzero_si128()), _mm_setzero_si128()));
				const T final_res3 = _mm_cvtsi128_si32(_mm_packus_epi16(_mm_packus_epi32(_mm_cvtps_epi32(hsum3), _mm_setzero_si128()), _mm_setzero_si128()));
			...
			else
			{
				...
				dst2[x] = _mm_cvtss_f32(_mm_hadd_ps(_mm_hadd_ps(result2, result2), _mm_hadd_ps(result2, result2)));
				dst3[x] = _mm_cvtss_f32(_mm_hadd_ps(_mm_hadd_ps(result3, result3), _mm_hadd_ps(result3, result3)));
				dst4[x] = _mm_cvtss_f32(_mm_hadd_ps(_mm_hadd_ps(result4, result4), _mm_hadd_ps(result4, result4)));
			}
